...
class CooDataStructure : public DataStructure {
 public:
    ...
    switch (nearest_power) {
      case 2:
        _impl = std::unique_ptr<DataStructure>(
            new CooDataStructureImpl<BaseType,
                                     static_cast<std::size_t>(2 * 1e2)>(
                row_indices, column_indices, values));
        break;
      case 3:
        _impl = std::unique_ptr<DataStructure>(
            new CooDataStructureImpl<BaseType,
                                     static_cast<std::size_t>(2 * 1e3)>(
                row_indices, column_indices, values));
        break;
      case 4:
        _impl = std::unique_ptr<DataStructure>(
            new CooDataStructureImpl<BaseType,
                                     static_cast<std::size_t>(2 * 1e4)>(
                row_indices, column_indices, values));
        break;
      case 5:
        _impl = std::unique_ptr<DataStructure>(
            new CooDataStructureImpl<BaseType,
                                     static_cast<std::size_t>(2 * 1e5)>(
                row_indices, column_indices, values));
        break;
      default:
  ...
  /** @brief Retrieve the IDs of all the nodes in the graph
   */
  std::vector<uint> allNodes(void) { return _impl->allNodes(); }
